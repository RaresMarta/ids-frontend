...
import { AlertTriangle, Ban, CheckCircle2, ShieldCheck, ShieldX } from 'lucide-react';
...
function bucket(evt: StreamEvent): number {
  ...
  return 0; // alert / ban / recovered are all incident-lifecycle events
}
...
function Row({ evt }: { evt: StreamEvent }) {
  switch (evt.type) {
    case 'flow': {
      ...
      return (
        <>
          <Icon className={`w-3.5 h-3.5 shrink-0 ${blocked ? 'text-[#DC4C4C]' : 'text-muted-foreground/50'}`} />
    ...
    case 'alert':
      return (
        <>
          <AlertTriangle className="w-3.5 h-3.5 shrink-0 text-[#DC4C4C]" />
          <span className="font-mono text-[11px] text-muted-foreground shrink-0">{fmtTime(evt.ts)}</span>
    ...
    case 'ban':
      return (
        <>
          <Ban className="w-3.5 h-3.5 shrink-0 text-[#DC4C4C]" />
          <span className="font-mono text-[11px] text-muted-foreground shrink-0">{fmtTime(evt.ts)}</span>
          <span className="text-[11px] text-foreground truncate flex-1">
            <span className="font-mono">{evt.attacker_ip}</span> banned
          </span>
          <FamilyChip family={evt.family} />
          <span className="font-mono text-[10px] text-muted-foreground w-9 text-right shrink-0">
            {evt.ttl_s}s
